...
namespace RMSDemo.Controllers
{
    public class AccountController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult UpdateUser(int Id, string FirstName, string UserName, string Password, string Email, string PhoneNumber)
        {
            try
            {
                Service service = new Service();
                ...
                service.UpdateUser(Id, Name[0], Name[1], UserName, Password, Email, PhoneNumber);
                return Json(new { Result = "OK" });
            }
            catch (Exception ex)
            {
